...
def main():
    ...
    train_y = []
    train_pic = []
    norm_y ,norm_name = load_image(train_norm_path, train_path,0, 0)
    train_y.extend(norm_y); train_pic.extend(norm_name)

    ad_y, ad_name = load_image(train_ad_path, train_path, 1, 1)
    train_y.extend(ad_y); train_pic.extend(ad_name)

    
    cur_path = '/data1/huahaoliu/cnn_traindata/caffe/mobile/201710/'
    train_fw = open(cur_path+'train.txt','w')
    for i in range(len(train_y)):
        train_fw.write(' '.join([train_pic[i], str(int(train_y[i]))]))
        train_fw.write('\n')
    train_fw.close()
    ...
    test_y = []
    test_pic = []
    norm_y ,norm_name = load_image(test_norm_path, test_path,0, 0)
    test_y.extend(norm_y); test_pic.extend(norm_name)

    ad_y, ad_name = load_image(test_ad_path, test_path, 1, 1)
    test_y.extend(ad_y); test_pic.extend(ad_name)

    

    cur_path = ''
    test_fw = open(cur_path+'test.txt','w')
    for i in range(len(test_y)):
        test_fw.write(' '.join([test_pic[i], str(int(test_y[i]))]))
        test_fw.write('\n')
    test_fw.close()
